import { Font, Template, checkTemplate } from "@pdfme/generator";
import { dogPngb64, examplePdfb64 } from "./sampleData";
...
export const getFont = () =>
	Promise.all(
		fonts.map((font) =>
			fetch(`/fonts/${font}.ttf`).then((res) => res.arrayBuffer())
		)
	).then((buffers) =>
		fonts.reduce(
			(acc, cur, index) =>
...
const divide = <T>(ary: T[], n: number): T[][] => {
	if (!Array.isArray(ary) || isNaN(n))
		throw Error("Error: divide unexpected argument"); // eslint-disable-line
	let idx = 0;
	const results = [];
...
export const normalizeDatas = (
	...
	const returnData = divide(datas, getLabelLengthInPage(template)).map(
		(labelsInPage) =>
